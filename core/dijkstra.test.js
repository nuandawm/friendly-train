...
const { elaboratePyramidFastestPath_simplifiedDijkstra, getVisitedParentNonVisitedNodes } = require("./dijkstra");
...
describe('get visited parent not visited nodes', () => {
  const node1 = SimpleNode(SimpleNodeType.SOURCE, 3, [])
  const node2 = SimpleNode(SimpleNodeType.MIDDLE, 7, [])
  const node3 = SimpleNode(SimpleNodeType.MIDDLE, 4, [])
  const node4 = SimpleNode(SimpleNodeType.DESTINATION, 2, [])
  const node5 = SimpleNode(SimpleNodeType.DESTINATION, 4, [])
